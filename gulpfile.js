...
function concatPlugins() {
	return gulp.src(['./js/plugins.*.js', './js/fn.*.js'])
		.pipe(concat('./plugins.js'))
		.pipe(replace('core.loadJS({ file: \'plugins', '//core.loadJS({ file: \'plugins'))
		.pipe(gulp.dest('./dist/js'));
}

function concatJS() {
	return gulp.src(['./js/functions.js', './dist/js/plugins.js'])
	.pipe(concat('./plugins.js'))
	.pipe(replace('jsLoadType: false', 'jsLoadType: \'fn\''))
	.pipe(uglify())
	.pipe(gulp.dest('./dist/js'));
}
...
exports.concatjs = series(concatPlugins, concatJS);
